...
/**
* Génère un bloc texte structuré pour analyse Claude.
*/
export function exportForClaude(data: AppData, startKey: string, endKey: string): string {
 ...
 const today = dateKey();
 const workoutRefs = new Map<string, { name: string; items: typeof (data.workouts)[0]['items']; fixedDuration?: string }>();
 ...
 const avgHabit = habitMax ? `${Math.round((habitTotal / habitMax) * (data.habits?.length || 0) * 10) / 10}/${data.habits?.length || 0}` : '-';
 ...
 for (const [, w] of workoutRefs) {
   ...
   const exercises = (w.items || []).map((it: any) => it.name || it.block).filter(Boolean).join(', ');
   if (exercises) wLine += `\n→ ${exercises}`;
