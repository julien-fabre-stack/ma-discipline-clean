...
function ExerciseEditor({
  ...
  return (
    <div className="rounded-2xl p-4 mb-3" style={{ background: C.surf2 }}>
...
export function ExerciseList({ items, onChange }: { items: Exercise[]; onChange: (items: Exercise[]) => void }) {
  ...
  if (edit != null && items[edit]) {
    return <ExerciseEditor ex={items[edit]} onChange={(p) => setEx(edit, p)} onClose={() => setEdit(null)} />;
  }
  ...
  return (
    <>
      ...
      {items.map((ex, i) => (
        <div key={ex.id} className="flex items-center gap-2 mb-2 px-3 py-2.5 rounded-xl" style={{ background: C.surf2 }}>
          ...
          <button onClick={() => setEdit(i)} className="p-1.5 rounded-lg" style={{ background: C.surf }}>
